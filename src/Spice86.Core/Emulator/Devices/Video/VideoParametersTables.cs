...
internal static class VideoParametersTables {
    private const int DefaultVideoParametersTableStaticAddress = 0xFF0A4;

    private static readonly FrozenSet<byte> video_parameter_table_vga = new byte[]{
...
// video parameter table for mode 13
  ...
  0x00, 0x00, 0x00, 0x00, 0x00, 0x40, 0x05, 0x0f, 0xff}.ToFrozenSet();  // graphics registers 0-8

    private static readonly FrozenSet<byte> _vparams = new byte[] {
    ...
    // CGA mode register
    0x2c, 0x28, 0x2d, 0x29, 0x2a, 0x2e, 0x1e, 0x29}.ToFrozenSet();

    public static void WriteBasicVideoParametersTable(IMemory memory) {
        for (ushort i = 0; i < _vparams.Items.Length; i++) {
            memory[(uint)DefaultVideoParametersTableStaticAddress + i] = _vparams.Items[i];
        }
    }

    public static void WriteVgaVideoParametersTable(IMemory memory, uint startAddress) {
        for (ushort i = 0; i <video_parameter_table_vga.Items.Length; i++) {
            memory[startAddress + i] = video_parameter_table_vga.Items[i];
        }
    }
}
