...
public class DiscriminatorReducer {
    ...
    public IList<CfgInstruction> ReduceAll(IList<CfgInstruction> instructions) {
        IDictionary<InstructionDiscriminator, IList<CfgInstruction>> groupedByDiscriminator =
    ...
    private void ReduceFieldValues(CfgInstruction reference, IList<CfgInstruction> instructions) {
        for (int i = 0; i < reference.FieldsInOrder.Count; i++) {
            IFieldWithValue referenceField = reference.FieldsInOrder[i];
    ...
    private bool IsFieldValueSame(IFieldWithValue reference, IList<CfgInstruction> instructions, int fieldIndex) {
        foreach (CfgInstruction instruction in instructions) {
            IFieldWithValue instructionField = instruction.FieldsInOrder[fieldIndex];
    ...
    private IDictionary<InstructionDiscriminator, IList<CfgInstruction>> GroupByDiscriminator(
        IList<CfgInstruction> instructions) {
        IEnumerable<IGrouping<InstructionDiscriminator, CfgInstruction>> grouped =
            instructions.GroupBy(i => i.Discriminator);
        return grouped.ToDictionary(g => g.Key, g => (IList<CfgInstruction>)g.ToList());
    }
}